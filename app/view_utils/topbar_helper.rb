module TopbarHelper
  ...
  def links(community:, user:, locale_param:, host_with_port:)
    gearwanted_link = if user.present?
                  payload_hash = {
                    email: user.confirmed_notification_email_to,
                    username: user.username,
                    admin: user.has_admin_rights?(community)
                  }.freeze
                 token = Base64.encode64(payload_hash.to_query)
                  "http://18.219.130.162/users/login_or_signup?token=#{token}"
                else
                  "http://18.219.130.162/users/login_or_signup"
                end
