...
class Category < ApplicationRecord
  ...
  def self.find_by_name(name)
    self.joins(:translations).where(category_translations: { name: name }).first
  end

  has_many :category_variable_custom_fields
  has_many :variable_custom_fields, through: :category_variable_custom_fields

  ####### Flying Flea Customizations #######
  MULTI_LISTING_CATEGORIES = ["Complete Rigs", "Partial Rigs", "Skydiving Gear Packages"]
  def is_multi_listing?
    MULTI_LISTING_CATEGORIES.include?(self.translations.find_by(:locale =>"en").name)
  end

  def is_full_rig?
    self.translations.find_by(:locale =>"en").name == "Complete Rigs"
  end

  def is_partial_rig?
    self.translations.find_by(:locale =>"en").name == "Partial Rigs"
  end

  def self.rig_category_ids
    self.where(url: ["containers", "main", "reserve", "aads"]).pluck(:id)
  end
end
