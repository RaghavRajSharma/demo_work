...
class Person < ApplicationRecord
  ...
  has_many :wishlist_listings, through: :wishlists, source: :listing
  ...
  def has_wishlist?(listing_id)
    wishlist_listings.pluck(:id).include?(listing_id)
  end
  ...
  def update_facebook_data(facebook_id, token)
    self.update(facebook_id: facebook_id, authentication_token: token)
  ...
  has_many :ratings, :through => :people_ratings

  has_many :people_disciplines 
  has_many :disciplines, :through => :people_disciplines
  belongs_to :favorite_discipline, :class_name => 'Discipline', :foreign_key => 'favorite_discipline_id'

  belongs_to :status
  belongs_to :home_dropzone, :class_name => 'DropZone', :foreign_key => 'favorite_dropzone_id'
  has_many :person_dropzones
  has_many :dropzones, :through => :person_dropzones, source: :dropzone

  def home_dropzone_name 
    home_dropzone && home_dropzone.name_and_location
  end

  def home_dropzone_name=(name_and_location)
    if name_and_location.present?
      #dropzone = DropZone.find_by(:name => name_and_location.split(' - ')[0])
      dropzone = DropZone.find_by(:id => name_and_location)
      self.home_dropzone = dropzone if dropzone
    end  
  end

  def generate_app_secret_proof=(access_token)
    secret_key = Community.first && Community.first.facebook_connect_secret
    hmac = OpenSSL::HMAC.new(secret_key, OpenSSL::Digest::SHA256.new)
    hmac << access_token
    proof = hmac.hexdigest
    self.app_secret_proof = proof
    self.save
  end

  def self.valid_login?(email, password)
    person = Email.find_by(address: email)&.person
    return nil unless person && person.valid_password?(password)
    person
  end

  def allow_token_to_be_used_only_once
    regenerate_api_token
    touch(:api_token_created_at)
  end
