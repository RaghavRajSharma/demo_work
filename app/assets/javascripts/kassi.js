...
function initialize_homepage() {
  ...
  $(".SearchBar__keywordInput__2HTav").autocomplete({
    source: "/suggestion",
    appendTo: ".SearchBar__focusContainer__2uI0-",
    minLength: 2,
    select: function( event, ui ) {
      $(".SearchBar__keywordInput__2HTav").val(ui.item.value);
      $(".SearchBar__searchButton__1Ck2b").trigger( "click" )

    }
  }).autocomplete( "instance" )._renderItem = function( ul, item ) {
    return $( "<li class='ui-menu-item'></li>" )
           .data( "item.autocomplete", item )
           .append( item.value + "<span class='category-text'> in " + item.category + "</span>" )
           .appendTo( ul );
  };
}
