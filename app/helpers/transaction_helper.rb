module TransactionHelper
    ...
    status_hash = {
      pending: ->() {
        ActiveSupport::Deprecation.warn("Transaction state 'pending' is deprecated and will be removed in the future.")
        {
          author: {
            ...
            text: I18n.t("conversations.status.waiting_for_you_to_accept_request")
          },
          starter: {
            icon: icon_waiting_other,
            text: I18n.t("conversations.status.waiting_for_listing_author_to_accept_request", listing_author_name: other_party_name)
          }
        } },
      preauthorized: ->() { {
        author: {
      ...
      accepted: ->() {
        ActiveSupport::Deprecation.warn("Transaction state 'accepted' is deprecated and will be removed in the future.")
          ...
          author: {
            icon: icon_waiting_other,
            text: I18n.t("conversations.status.waiting_payment_from_requester", requester_name: other_party_name)
          },
          starter: {
            icon: icon_waiting_you,
            text: I18n.t("conversations.status.waiting_payment_from_you")
          }
        } },
      pending_ext: ->() {
        ...
        when "multicurrency"
          {
            author: {
              ...
              text: I18n.t("conversations.status.pending_external_inbox.paypal.multicurrency")
            },
            starter: {
              icon: icon_waiting_other,
              text: I18n.t("conversations.status.waiting_for_listing_author_to_accept_request", listing_author_name: other_party_name)
        ...
        when "intl"
          {
            author: {
              icon: icon_waiting_you,
              # Make this aware of the reason
              text: I18n.t("conversations.status.pending_external_inbox.paypal.intl")
            },
            starter: {
              icon: icon_waiting_other,
              text: I18n.t("conversations.status.waiting_for_listing_author_to_accept_request", listing_author_name: other_party_name)
        ...
        when "verify"
          {
            author: {
              icon: icon_waiting_you,
              # Make this aware of the reason
              text: I18n.t("conversations.status.pending_external_inbox.paypal.verify")
            },
            starter: {
              icon: icon_waiting_other,
              text: I18n.t("conversations.status.waiting_for_listing_author_to_accept_request", listing_author_name: other_party_name)
        ...
        else # some pending_ext reason we don't have special message for
          {
            author: {
              icon: icon_waiting_you,
              text: I18n.t("conversations.status.pending_external_inbox.paypal.unknown_reason")
            },
            starter: {
              icon: icon_waiting_other,
              text: I18n.t("conversations.status.waiting_for_listing_author_to_accept_request", listing_author_name: other_party_name)
      ...
      rejected: -> () { {
        both: {
          ...
          text: I18n.t("conversations.status.request_rejected")
        }
      } },
      ...
      paid: ->() { {
        author: {
          icon: icon_waiting_other,
          text: I18n.t("conversations.status.waiting_confirmation_from_requester", requester_name: other_party_name)
        },
        starter: {
          icon: icon_waiting_you,
          text: I18n.t("conversations.status.waiting_confirmation_from_you")
        }
      } },
      ...
      waiting_feedback: ->() { {
        both: {
          icon: icon_waiting_you,
          text: I18n.t("conversations.status.waiting_feedback_from_you")
        }
      } },
      ...
      completed: ->() { {
        both: {
          ...
          text: I18n.t("conversations.status.request_confirmed")
        }
      } },
      ...
      canceled: ->() { {
        both: {
          ...
          text: I18n.t("conversations.status.request_canceled")
        }
      } },
      ...
      errored: ->() { {
        both: {
          icon: icon_tag("cross", ["icon-fix-rel", "canceled"]),
          text: I18n.t("conversations.status.payment_errored")
         }
      } },
    }
  ...
  def get_conversation_statuses(conversation, is_author)
    statuses = if conversation.listing && !conversation.status.eql?("free")
      status_hash = {
        paid: ->() { {
          both: [
            status_info(I18n.t("conversations.status.request_paid"), icon_classes: icon_for("paid")),
        ...
        preauthorized: ->() { {
          both: [
            status_info(I18n.t("conversations.status.request_preauthorized"), icon_classes: icon_for("preauthorized")),
        ...
        pending_ext: ->() {
          ...
          when Some("multicurrency")
            {
              author: [
                status_info(I18n.t("conversations.status.pending_external.paypal.multicurrency", currency: paypal_payment[:payment_total].currency, paypal_url: link_to("https://www.paypal.com", "https://www.paypal.com")).html_safe, icon_classes: icon_for("pending_ext"))
              ],
              starter: [
                status_info(I18n.t("conversations.status.request_preauthorized"), icon_classes: icon_for("preauthorized")),
                preauthorized_status(conversation)
          ...
          when Some("intl")
            {
              author: [
                status_info(I18n.t("conversations.status.pending_external.paypal.intl", paypal_url: link_to("https://www.paypal.com", "https://www.paypal.com")).html_safe, icon_classes: icon_for("pending_ext"))
              ],
              starter: [
                status_info(I18n.t("conversations.status.request_preauthorized"), icon_classes: icon_for("preauthorized")),
                preauthorized_status(conversation)
          ...
          when Some("verify")
            {
              author: [
                status_info(I18n.t("conversations.status.pending_external.paypal.verify", paypal_url: link_to("https://www.paypal.com", "https://www.paypal.com")).html_safe, icon_classes: icon_for("pending_ext"))
              ],
              starter: [
                status_info(I18n.t("conversations.status.request_preauthorized"), icon_classes: icon_for("preauthorized")),
                preauthorized_status(conversation)
        ...
        confirmed: ->() { {
          both: [
            status_info(I18n.t("conversations.status.request_confirmed"), icon_classes: icon_for("confirmed")),
            feedback_status(conversation)
          ]
        } },
        canceled: ->() { {
          both: [
            status_info(I18n.t("conversations.status.request_canceled"), icon_classes: icon_for("canceled")),
            feedback_status(conversation)
          ]
        } },
        rejected: ->() { {
          both: [
            status_info(I18n.t("conversations.status.request_rejected"), icon_classes: icon_for(conversation.status))
          ]
        } },
        errored: ->() { {
          author: [
            status_info(I18n.t("conversations.status.payment_errored_author", starter_name: conversation.starter.name(conversation.community)), icon_classes: icon_for("errored"))
          ],
          starter: [
            status_info(I18n.t("conversations.status.payment_errored_starter"), icon_classes: icon_for("errored"))
          ]
        } }
      }
  ...
  def delivery_status(conversation)
    if current_user?(conversation.author)
      status_info(
        I18n.t("conversations.status.waiting_for_current_user_to_deliver_listing",
      ...
      status_info(
        I18n.t("conversations.status.waiting_for_listing_author_to_deliver_listing",
          :listing_title => link_to(conversation.listing.title, conversation.listing),
    ...
    status_links([
      {
        ...
        link_text_with_icon: I18n.t("conversations.status.give_feedback")
      },
      {
        ...
        link_text_with_icon: I18n.t("conversations.status.skip_feedback"),
  ...
  def link_text_with_icon(conversation, status_link_name)
    if ["accept", "reject", "accept_preauthorized", "reject_preauthorized"].include?(status_link_name)
      I18n.t("conversations.status_link.#{status_link_name}_request")
    else
      I18n.t("conversations.status_link.#{status_link_name}")
    end
  end

  def waiting_for_buyer_to_confirm(conversation)
    link = I18n.t("conversations.status.waiting_confirmation_from_requester",
  ...
  def waiting_for_author_to_accept_preauthorized(transaction)
    text = I18n.t("conversations.status.waiting_for_listing_author_to_accept_request",
  ...
  def feedback_given_status
    status_info(I18n.t("conversations.status.feedback_given"), icon_tag: icon_tag("testimonial", ["icon-part"]))
  end

  def feedback_skipped_status
    status_info(I18n.t("conversations.status.feedback_skipped"), icon_classes: "ss-skipforward")
  end
