...
class HomepageController < ApplicationController
  ...
  def index
    ...
    filter_params[:categories] = parse_categories_fields(params) if parse_categories_fields(params).present?
    ...
    if FeatureFlagHelper.feature_enabled?(:searchpage_v1)
      ...
      @big_cover_photo = @current_community.cover_enabled? && (!(@current_user || CustomLandingPage::LandingPageStore.enabled?(@current_community.id)) || params[:big_cover_photo])
    ...
    if FeatureFlagHelper.feature_enabled?(:searchpage_v1)
      ...
      search_result.on_success { |listings|
        ...
        if @view_type == "grid" ||  @view_type == "list" then
          if @view_type == "grid" && current_page > 1
            render partial: "grid_item", collection: @listings, as: :listing, locals: { show_distance: location_in_use }
          elsif @view_type == "list" && current_page > 1
          ...
          # render layout: false, partial: "list_item", collection: @listings, as: :listing, locals: { shape_name_map: shape_name_map }
        end
      }.on_error {
  ...
  def suggestion
    # listings = Listing.search(Riddle::Query.escape(params[:term]), star: true) if params[:term]
    if params[:term].present?
      categry_ids = Category.all.select{|cat| cat.display_name(I18n.locale).downcase.include?(params[:term].downcase) || params[:term].downcase.include?(cat.display_name(I18n.locale).downcase)}.map(&:id)
      if categry_ids.present?
        subcategories_ids = Category.where(id: categry_ids).map{|cat| cat.subcategories.pluck(:id)}.flatten
        category_listings = Listing.currently_open.where(category_id: (subcategories_ids + categry_ids).uniq)
      else
        category_listings = []
      end
      listings = Listing.currently_open.where("lower(title) LIKE ?", "%#{params[:term].downcase}%")
      listings = listings + category_listings
      suggestions = listings.map{|l| {id: l.id, value: l.title, category: l.category.display_name(I18n.locale), category_id: l.category_id} }
      render json: suggestions, root: false
    else
      render json: {}
    end
  end
  ...
  def parse_categories_fields(params)
    params.select { |key, value|
      key.start_with?('category_')
    }.values.map(&:to_i)
  end
  ...
  def find_listings(params:, current_page:, listings_per_page:, filter_params:, includes:, location_search_in_use:, keyword_search_in_use:, relevant_search_fields:)
    unless params[:q].present?
      ...
      listings = Listing.currently_open.where("lower(title) LIKE ?", "%#{params[:q].downcase}%")
      search = {
