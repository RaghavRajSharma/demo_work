...
class SessionsController < ApplicationController
  ...
  def new
    return redirect_to single_signon_url if @current_user && params[:sso].present?
  ...
  def create
    ...
    @current_user = person
    return redirect_to single_signon_url if params[:sso].present?
    ...
    #flash[:notice] = t("layouts.notifications.login_successful")
  ...
  def facebook
    ...
    if person
      person.update_facebook_data(data.id, access_token)
      person.generate_app_secret_proof = access_token
      person.authentication_token = access_token
  ...
  def facebook_setup
    ...
    request.env["omniauth.strategy"].options[:scope] = "public_profile,email,user_friends"
  ...
  def single_signon_url
    decode_sso = Base64.decode64(params[:sso])
    query_string = Rack::Utils.parse_query(decode_sso)
    payload_hash = {
      ...
      admin: @current_user.has_admin_rights?(@current_community)
    }.freeze
    encoded_payload = Base64.strict_encode64(payload_hash.to_query)
    secured_hash = OpenSSL::HMAC.hexdigest("SHA256", APP_CONFIG.sso_secret_key, encoded_payload)
    "#{query_string['return_sso_url']}?sso=#{encoded_payload}&sig=#{secured_hash}"
  end
