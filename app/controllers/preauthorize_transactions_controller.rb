...
class PreauthorizeTransactionsController < ApplicationController
  ...
  def initiate
    @shipment = calculate_shipping_amount(listing) if params[:delivery] == 'shipping'
    ...
    validation_result.on_success { |tx_params|
      ...
      render "listing_conversations/initiate",
             locals: {
               ...
               stripe_shipping_required: params[:delivery] == 'shipping' && tx_params[:delivery] != :pickup,
               ...
               shipping_address_locals:  session[:shipping_address]
             }
    }
  ...
  def initiated
    params[:shipping_address] = session[:shipping_address] if session[:shipping_address].present?
    ...
    validation_result.on_success { |tx_params|
      ...
      if params[:shipment_rate_id].present?
        ...
        if @shippo_transaction.status == 'ERROR'
          flash[:error] = @shippo_transaction.messages.last["text"]
          render :js => "window.location = '/listings/#{listing.id}'"
          return
        end
      end
  ...
  def select_shipping_rate
    price_break_down_locals = eval params[:price_break_down_locals]
    price_break_down_locals.each do |k,v|
      price_break_down_locals[k] = Money.new(v*100, @current_community.currency) if v.class.to_s == 'Float'
    end
    price_break_down_locals[:shipping_price] = Money.new(params[:ship_rate].to_f*100, @current_community.currency)
    price_break_down_locals[:total] = price_break_down_locals[:subtotal] + price_break_down_locals[:shipping_price] + price_break_down_locals[:tax]
    render layout: false, locals: { price_break_down_locals: price_break_down_locals}
  end
  ...
  def calculate_shipping_amount(listing)
    ...
    address_to[:phone] = @current_user.phone_number if @current_user.phone_number.present?
    parcel = {
