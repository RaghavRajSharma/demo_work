class ListingsController < ApplicationController
  ...
  skip_before_action :verify_authenticity_token, :only => [:close, :update, :follow, :unfollow, :will_or_not_willing_to_piece]
  ...
  def show
    ...
    @friends_in_common = @fb.mutual_friends(@listing.author.authentication_token) if @fb && !current_user?(@listing.author) && @listing.author.authentication_token.present?
    record_event(
      flash.now,
      "ListingViewed",
  ...
  def create
    ...
    parent_listing = Listing.find_by(id: params[:parent_id]) if params[:parent_id] != ""
    if parent_listing
    ...
    ActiveRecord::Base.transaction do
      ...
      if @listing.save
        ...
        @listing.listing_open_close_for_complete_rig
        if parent_listing
          parent_listing.listing_open_close_for_complete_rig
        end
        ...
        if shape[:availability] == :booking
          ...
          if @listing.parent_id
            redirect_to parent_listing, status: 303
          else
  ...
  def add_child_listings_to_multi_listing
    @parent = Listing.find_by(id: params[:parent_id])
    if @parent.author != @current_user 
      flash[:error] = "Can't add listings to another user's multi-listing"
      redirect_to @parent
    end
    ...
    if params[:type] &&
        params[:type].to_i != 0 &&
        params[:listing] &&
        params[:listing][:new_child] &&
        category = Category.find_by(id: params[:type].to_i)

      @parent.new_child = params[:listing][:new_child]
      category_name = category.display_name('en').downcase
      if category_name == 'containers'
        render layout: false, :partial => 'listings/child_container_listing_links', :locals => { :listing => @parent }
      elsif category_name == 'main'
        render layout: false, :partial => 'listings/child_main_listing_links', :locals => { :listing => @parent }
      elsif category_name == 'reserve'
        render layout: false, :partial => 'listings/child_reserve_listing_links', :locals => { :listing => @parent }
      elsif category_name == 'aads'
        render layout: false, :partial => 'listings/child_aad_listing_links', :locals => { :listing => @parent }
      end
    else
      @parent.child_ids = params[:listing][:child_ids]
      unless request.xhr?
        redirect_to listing_path(@parent.id)
        return
      else
        render :js => "window.location = #{listing_path(@parent.id)}"
        return
      end
    end
    @parent.listing_open_close_for_complete_rig
  end

  def rig_category_ids
    render json: Category.rig_category_ids
  end

  def remove_listing_from_rig
    @parent = Listing.find_by(id: params[:parent_id])
    @child = Listing.find_by(id: params[:id])
    if @parent && @child && @child.parent_id == @parent.id && @parent.author == @current_user && @child.author == @current_user 
      @child.parent_id = nil
      @child.save
      if params[:type] == 'container'
        ...
        render layout: false, :partial => 'listings/child_main_listing_links', :locals => { :listing => @parent }
      elsif params[:type] == 'reserve'
        render layout: false, :partial => 'listings/child_reserve_listing_links', :locals => { :listing => @parent }
      elsif params[:type] == 'aad'
        render layout: false, :partial => 'listings/child_aad_listing_links', :locals => { :listing => @parent }
      end
    else 
      flash[:error] = "Can't remove another user's listing from multi-listing"
      unless request.xhr?
        redirect_to listing_path(@parent.id)
        return
      else
  ...
  def listing_suggestion
    if params[:term].present? && (params[:category].present? || params[:subcategory].present?)
      ...
      render json: titles, root: false
    else
      render json: {}
    end
  end

  def validate_listing_title
    if params[:listing][:title].present? && (params[:category].present? || params[:subcategory].present?)
      category = params[:subcategory].present? ? params[:subcategory] : params[:category]
      titles = ListingTitleService.new(category).search_title_options(params[:listing][:title])
      respond_to do |format|
        format.json { render :json => (titles.present? ? titles.include?(params[:listing][:title]) : false), root: false }
      end
    else
      render json: true
    end   
  ...
  def will_or_not_willing_to_piece
    ...
    listing.update_column(:willing_to_piece, !listing.willing_to_piece)
    render json: true
  end
