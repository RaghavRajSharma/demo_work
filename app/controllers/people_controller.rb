class PeopleController < Devise::RegistrationsController
  ...
  def show
    ...
    @friends_in_common = @fb.mutual_friends(@person.authentication_token) if @fb && !current_user?(@person) && @person.authentication_token
  ...
  def create_facebook_based
    ...
    person_hash = {
      ...
      :authentication_token => session["devise.facebook_data"]["authentication_token"],
      :locale => I18n.locale,
  ...
  def update
    ...
    begin
      ...
      person_params.merge!(dropzone_ids: []) unless person_params[:dropzone_ids]
      ...
      if target_user.update_attributes(person_params.except(:email_attributes))
        ...
        # Update profession for "Other"
        if person_params[:profession].present?  && person_params[:profession].eql?("Other")
          @person.update_attribute(:profession, params[:profession])
        end
