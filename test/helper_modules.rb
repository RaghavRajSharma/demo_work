...
module TestHelpers
  ...
  def load_product_data 
    canopies_category = find_category_by_name("Canopies")
    require_relative '../db/seed_helpers/data/canopies'
    canopies.each do |canopy| 
      Product.new_from_hash(canopy)
    end
    require_relative '../db/seed_helpers/data/containers'
    containers.each do |container|
      Product.new_from_hash(container)
    end
    require_relative '../db/seed_helpers/data/aads'
    aads.each do |aad|
      Product.new_from_hash(aad)
    end
  end

  module_function :load_product_data
