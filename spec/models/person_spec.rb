...
describe Person, type: :model do
  ...
  describe "ratings and disciplines" do 
    let(:person) { FactoryGirl.create(:person) }
    let(:rating) { FactoryGirl.create(:rating) }
    let(:discipline) { FactoryGirl.create(:discipline) }

    it "has many ratings through people_ratings" do 
      pr = PeopleRating.create(:person_id => person.id, :rating_id => rating.id)
      expect(person.people_ratings).to include(pr)
      expect(person.ratings).to include(rating)
    end

    it "has many disciplines through people_disciplines" do 
      pd = PeopleDiscipline.create(:person_id => person.id, :discipline_id => discipline.id)
      expect(person.people_disciplines).to include(pd)
      expect(person.disciplines).to include(discipline)
    end
  end
